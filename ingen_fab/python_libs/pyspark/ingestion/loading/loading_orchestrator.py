...
class LoadingOrchestrator:
    ...
    def _discover_batches(self, config: ResourceConfig) -> List[BatchInfo]:
        """
        ...
        logger.info(f"Discovered {len(batches)} batches from extraction logs")
        return batches

    def _recover_stale_batches(self, config: ResourceConfig) -> None:
        """
        ...
        logger.warning(f"Found {len(stale_rows)} stale batches, recovering...")
        ...
        for row in stale_rows:
            ...
            logger.info(f"Recovered batch {batch_id}")
    ...
    def process_resources(
        ...
        Args:
            ...
            is_retry: If True, only process configs whose last load failed
        ...
        if is_retry:
            failed_keys = self.logger_instance.get_failed_resource_keys(configs)
        ...
        logger.info(
            f"Loading complete: {success_count} successful, "
    ...
    def _process_group_parallel(
        ...
        logger.info(
            f"{len(results)} resource(s) - {success_count} success, {warning_count} warning, {error_count} error, {rejected} with rejections"
        )
        ...
        for result in sorted_results:
            ...
            if result.status == ExecutionStatus.ERROR and result.error_message:
                ...
                logger.info(f"{icon} {result.resource_name} (error) - {result.error_message}")
            else:
                # Success or warning - log status
                logger.info(
    ...
    def process_single_resource(
        ...
        """
        # Set logging context (automatically prefixes all log messages)
        ...
        try:
            logger.info(f"Loading: {config.resource_name}")
            ...
            logger.info(
                ...
                f"({config.extract_file_format_params.file_format}, {config.target_write_mode})"
            )
            ...
            self._recover_stale_batches(config)

            # Step 2: Discover batches from extraction logs (dlt-style work queue)
            discovered_batches = self._discover_batches(config)
            result.batches_discovered = len(discovered_batches)
            ...
            self._finalize_result(result, all_metrics, config, start_time)
            ...
            result.status = ExecutionStatus.ERROR
            result.error_message = str(e)
            logger.exception(f"Unexpected error in loading: {e}")
            self.logger_instance.log_resource_execution_error(config, execution_id, load_run_id, str(e), result)
    ...
    def _process_batches(
        ...
        Args:
            ...
            ctx: BatchContext with config, execution_id, load_run_id, file_loader, logger
        ...
        for batch_index, batch_info in enumerate(batches, start=1):
            try:
                ...
                # Log completion to database based on data quality
                if combined_metrics.corrupt_records_count > 0:
                    # Data quality issues were filtered - log as warning
                    self.logger_instance.log_batch_warning(
                        ctx.config,
                        ctx.execution_id,
                        ctx.load_run_id,
                        batch_info.batch_id,
                        f"Data quality issues filtered: {combined_metrics.corrupt_records_count} corrupt records",
                        batch_info,
                        metrics=combined_metrics
                    )
                    # Update extraction log (PENDING → WARNING)
                    ...
                    # Update extraction log (PENDING → SUCCESS)
                    self.logger_instance.update_extraction_batch_load_state(
                        batch_info.extract_batch_id,
                        ctx.config.source_name,
                        ctx.config.resource_name,
                ...
                self._handle_batch_error(error, batch_info)
    ...
    def _process_single_batch(
        ...
        if raw_result.status == "failed":
            ...
            logger.error(f"Failed to load files to staging table: {error_message}")
        ...
        logger.info(f"Step 1/2: Loaded {raw_rows:,} rows to staging table")
        ...
        if result.status == "rejected":
            ...
            logger.info(f"Step 2/2: Batch rejected: {file_name}")
        ...
        logger.info(f"Step 2/2: Wrote {target_rows:,} rows to target table")
    ...
    def _move_rejected_batch_to_error(
        ...
        - Multi-file batches (batch_by="all" or batch_by="folder")
        ...
        """
        # Validate we have files to move
        if not batch_info.file_paths:
            logger.warning("Cannot move rejected batch - no file_paths in BatchInfo")
            return

        # Move all files in the batch (handles batch_by="all" and batch_by="folder")
        files_moved = 0
        for source_path in batch_info.file_paths:
            # Skip folder paths (end with /)
            ...
            logger.info(f"Moving rejected file: {source_path} → {error_path}")
            ...
            if data_moved:
                files_moved += 1
                logger.debug(f"Data file moved successfully: {os.path.basename(source_path)}")

                # Move control file if configured (best-effort)
                self._move_control_file_to_error(source_path, error_path, ctx)
            else:
                logger.warning(f"Failed to move data file: {os.path.basename(source_path)}")

        # Log summary
        if files_moved > 0:
            logger.info(f"Moved {files_moved} file(s) to error location")
        else:
            logger.warning("No files were moved to error location")
    ...
    def _move_control_file_to_error(
        ...
        if isinstance(ctx.config.source_extraction_params, dict):
            control_pattern = ctx.config.source_extraction_params.get("control_file_pattern")
        else:
            control_pattern = getattr(ctx.config.source_extraction_params, "control_file_pattern", None)
        ...
        try:
            ...
            if not control_source or not control_error:
                logger.debug("Could not derive control file paths - skipping control file move")
                return
            ...
            if control_moved:
                ...
                logger.debug(f"Control file moved: {control_filename}")
            else:
                # File might not exist or already moved - log at debug level
                logger.debug("Control file not found or could not be moved")

        except Exception as e:
            # Control file movement is best-effort - log and continue
            logger.debug(f"Could not move control file: {e}")
    ...
    def _handle_batch_error(
        ...
        if isinstance(error, WriteError):
            logger.exception(f"Batch {load_id} write failed: {error_msg}")
        else:
            logger.exception(f"Batch {load_id} failed: {error_msg}")
    ...
    def _finalize_result(
        ...
        # Note: If we reach here, no exceptions were raised (loading stops on first error)
        if result.batches_processed > 0:
            # All batches succeeded (rejections don't cause failure)
            result.status = ExecutionStatus.SUCCESS
        else:
            # Defensive case: shouldn't happen since we check for empty discovered_batches earlier
            result.status = ExecutionStatus.SUCCESS
        ...
        if all_metrics:
            ...
            logger.info(f"Completed: {batch_text} in {duration_s:.1f}s")
